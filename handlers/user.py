...
async def time_handler_correct(
    ...
    await message.answer(text=i18n.hosp_name())
...
async def hosp_name_handler_correct(
    ...
    await message.answer(text=i18n.inc_number())
    await state.set_state(FSMFillIncident.inc_number)


@router.message(StateFilter(FSMFillIncident.hosp_name))
async def hosp_name_handler_incorrect(message: Message, i18n: TranslatorRunner):
    await message.answer(text=i18n.hosp_name_incorrect())
...
async def inc_number_handler_correct(
    ...
    await message.answer(text=i18n.sti_res(), reply_markup=sti_res_kb())
    await state.set_state(FSMFillIncident.sti_res)


@router.message(StateFilter(FSMFillIncident.inc_number))
async def inc_number_handler_incorrect(message: Message, i18n: TranslatorRunner):
    await message.answer(text=i18n.inc_number_incorrect())
...
async def sti_res_handler_yes(
    ...
    if callback.message:
        await callback.message.answer(text=i18n.inc_child_number())
...
async def inc_child_number_handler_correct(
    ...
    await message.answer(text=i18n.description())
    await state.set_state(FSMFillIncident.description)


@router.message(StateFilter(FSMFillIncident.inc_child_number))
async def inc_child_number_handler_incorrect(message: Message, i18n: TranslatorRunner):
    await message.answer(text=i18n.inc_number_incorrect())
...
async def sti_res_handler_no(
    ...
    if callback.message:
        await callback.message.answer(text=i18n.description())
        await state.set_state(FSMFillIncident.description)
...
async def description_handler(
    ...
    await message.answer(text=i18n.resolution())
...
async def refil_btn_process(
    ...
    if callback.message:
        await callback.message.answer(
            text=i18n.add_incident(current_time=await state.get_value("current_time"))
        )
        await state.clear()
        await state.set_state(FSMFillIncident.time)
...
async def confirm_btn_process(
    ...
    if callback.message:
        await callback.message.answer(text=i18n.confirm_pressed())
